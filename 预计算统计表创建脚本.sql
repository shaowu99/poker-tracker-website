...
CREATE TABLE IF NOT EXISTS player_stats_summary (
  ...
  aggression_index DECIMAL(5, 2) DEFAULT 0, -- 激进指数
...
BEGIN
  ...
  -- 计算胜率和平均每手盈利
  win_rate_calc := CASE WHEN total_hands_count > 0 THEN (total_wins * 100.0 / total_hands_count) ELSE 0 END;
  avg_profit_calc := CASE WHEN total_hands_count > 0 THEN (total_profit_sum / total_hands_count) ELSE 0 END;

  -- 计算VPIP - 自愿投入底池率
  SELECT 
    COALESCE(COUNT(DISTINCT ha.game_id) * 100.0 / NULLIF(total_hands_count, 0), 0)
  INTO vpip_calc
  ...
  -- 简化的激进指数
  aggression_calc := CASE WHEN vpip_calc > 0 THEN (pfr_calc / NULLIF(vpip_calc, 0)) ELSE 0 END;
...
BEGIN
  ...
  SELECT 
    ...
    (SUM(CASE WHEN is_winner THEN 1 ELSE 0 END) * 100.0 / COUNT(*)) as win_rate
  FROM player_positions
...
-- 9. 为player_positions表创建触发器
CREATE TRIGGER IF NOT EXISTS trigger_update_player_stats
...
-- 11. 为hand_actions表创建触发器
CREATE TRIGGER IF NOT EXISTS trigger_update_player_actions_stats
